 ...
 *   GET    /api/medical-photos/{id}                   → Metadata + URL presignada
 *   GET    /api/medical-photos/{id}/download          → Descargar archivo binario
 ...
 *
 * Upload: multipart/form-data con campo "file" + campos de metadata.
 *
 * El mapper a PhotoResponse vive en MedicalPhotoService.toResponse() porque
 * necesita acceso a MediaStorageService para generar URLs presignadas.
 */
@RestController
@RequestMapping("/api")
public class MedicalPhotoController {
	...
	public ResponseEntity<PhotoResponse> upload(
		...
		return ResponseEntity.status(HttpStatus.CREATED).body(photoService.toResponse(saved));
	}

	// ─── READ (metadata + URL presignada) ────────────────────────────
	...
	public ResponseEntity<PhotoResponse> getById(
		...
		return ResponseEntity.ok(photoService.toResponse(photo));
	}

	// ─── DOWNLOAD (binario) ──────────────────────────────────────────

	/**
	 * Descarga binaria del archivo. Usado para casos donde el frontend
	 * NO puede usar la URL presignada directamente (ej: descargar como
	 * adjunto, exportación a PDF, generación de zip).
	 *
	 * Para previsualización inline, el frontend debe usar la URL del
	 * campo `url` del PhotoResponse (más rápido, sin pasar por el backend).
	 */
